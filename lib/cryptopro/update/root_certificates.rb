  ...
  module Update
    class RootCertificates
      ...
      def self.download(array_of_links, output_folder)
        ...
        temp_file = Tempfile.new('root_certificates')
        array_of_links.each{ |link| temp_file.puts link }
        temp_file.rewind

        # We're using wget to download all necessary root certificates.
        logger = Logger.new('log/root_cert_download.log')
      ...
      def self.install(output_folder)
        ...
        # First of all we're looking for certificate files and checking if they are of a fresh version.
        root_certificate_files = Dir["#{output_folder}/*"]
        root_certificate_files = root_certificate_files.select{ |file| File.ctime(file).today? }
        ...
        # We're using part of a CryptoPro CSP toolkit: certmgr, to install all our root certificates to root certificate
        # store and we're going to send any errors we encounter to a log file.
        logger = Logger.new('log/root_cert_install.log')
        line = Cocaine::CommandLine.new(
            'certmgr', '-inst -store root -file :file',
            logger: logger
        )

        root_certificate_files.each do |file|
